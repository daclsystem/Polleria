  ...
  return (
    <div>
      <div className="flex flex-wrap items-end justify-between gap-3">
        <PageTitle title="Inventario" hint="Control de insumos. Rojo = bajo el mínimo." />
        <button
          ...
          return (
            <article key={i.id} className={`card p-4 ${low ? 'ring-1 ring-brick/30' : ''}`}>
              ...
              <div className="mt-3 flex gap-1">
                <button className="min-h-10 rounded-lg bg-cream px-3" onClick={() => adjustStock(i.id, -1)}>
                  −1
                </button>
                <button className="min-h-10 rounded-lg bg-cream px-3" onClick={() => adjustStock(i.id, 1)}>
                  +1
                </button>
                <button className="min-h-10 rounded-lg bg-cream px-3" onClick={() => adjustStock(i.id, 5)}>
                  +5
                </button>
              </div>
            </article>
                ...
                <tr key={i.id} className={`border-t border-ink/5 ${low ? 'bg-rose-50' : ''}`}>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-1">
                      <button className="rounded-lg bg-cream px-2 py-1" onClick={() => adjustStock(i.id, -1)}>
                        −1
                      </button>
                      <button className="rounded-lg bg-cream px-2 py-1" onClick={() => adjustStock(i.id, 1)}>
                        +1
                      </button>
                      <button className="rounded-lg bg-cream px-2 py-1" onClick={() => adjustStock(i.id, 5)}>
                        +5
                      </button>
                    </div>
                  </td>
